...
export class DataService {
    ...
    public async createWalletAddresses(id: string, wallet: WalletAddressRequestDTO): Promise<ApiResponse<WalletAddressResponseDTO>> {
        return new Promise((resolve, reject) => {
            this.apiManager.post<WalletAddressResponseDTO>(`${CommonFunctions.StringFormat(Constants.GenerateWalletAddresses, id)}`, wallet).subscribe((s) => {
                resolve(ResponseHelper.CreateResponse<WalletAddressResponseDTO>(CommonFunctions.StringFormat(Messages.successPOST, "Wallet Addresses"), s.data, s.status))
            }, (e) => {
                reject(ResponseHelper.CreateResponse<WalletAddressResponseDTO>(CommonFunctions.StringFormat(Messages.failurePOST, "Wallets Addresses"), e, e.response.status))
            })
        })
    }
     ...
     * @returns {WalletResponseDTO} WalletResponseDTO response from circle api
     */
    public async getWalletAddresses(id: string, wallet: WalletAddressRequestDTO): Promise<ApiResponse<WalletAddressResponseDTO>> {
        return new Promise((resolve, reject) => {
            this.apiManager.get<WalletAddressResponseDTO>(`${CommonFunctions.StringFormat(Constants.GenerateWalletAddresses, id)}`).subscribe((s) => {
                ...
                reject(ResponseHelper.CreateResponse<WalletAddressResponseDTO>(CommonFunctions.StringFormat(Messages.failureGET, "Wallets Addresses"), e, e.response.status))
            })
        })
    }
